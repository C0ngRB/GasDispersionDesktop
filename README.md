...
- 生成：`cmake -S . -B build-mingw -G "MinGW Makefiles" -DCMAKE_PREFIX_PATH=C:/Qt/6.9.3/mingw_64`
- 构建：`cmake --build build-mingw -- -j 8`

### 9.2 使用 Qt MSVC 套件
- 需要安装对应的 Qt MSVC 版本（不是 mingw_64）。
- 生成：`cmake -S . -B build -G "Visual Studio 17 2022" -A x64 -DCMAKE_PREFIX_PATH=<Qt MSVC path>`
